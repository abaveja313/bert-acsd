...
def load_category_examples(path: str = "./data/semeval/labeled_training.bin") -> List[LabeledExample]:
    """Read cached file"""
    try:
        with open(path, 'rb') as f:
            examples = pickle.load(f)
            logger.info("Loaded " + len(examples) + " examples from data path '" + path + "'")
    except FileNotFoundError:
        raise Exception("Training example dataset has not yet been generated! Call ./data/semeval/adapter.py")
